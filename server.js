...
import { ethers } from "ethers"; // Simplified import
...
let provider, wallet, faucetContract;
...
app.get("/api/config", (_req, res) => {
  res.json({
    ok: true,
    chainIdHex: process.env.CHAIN_ID_HEX || "0x1",
    tokenSymbol: process.env.TOKEN_SYMBOL || "OGMN",
    token: {
      address: process.env.TOKEN_ADDRESS || "",
      decimals: Number(process.env.TOKEN_DECIMALS || 18),
    },
    contract: process.env.GAME_CONTRACT || "",
  });
});
